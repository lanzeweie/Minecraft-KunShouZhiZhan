...
execute as @e[tag=airdrop] at @s[gamemode=survival] run spreadplayers ~ ~ 30 60 false @s

# 确保盔甲架不会出现在水中，如果出现在水中，则重新分散
execute as @e[tag=airdrop] at @s[gamemode=survival] unless block ~ ~-38 ~ water run spreadplayers ~ ~ 30 60 false @s

# 确保盔甲架在玩家上方80格位置
execute as @e[tag=airdrop] at @s[gamemode=survival] run tp @s ~ ~80 ~
...
# 15分钟后重新运行此函数
schedule function hanhankszz:airboot_time/pure_airboot45min 270s
